...
/*
 * The btree key for each xattr is { hash of name, unique counter value,
 * 0 } so once we have iterated over all the keys with a matching hash
 * but no matching name, as signified by calling the callback with a
 * null val pointer, we can assume that nothing with that name exists.
 */
 ...
 * negative. The three cases are:
 *
 * - remove xattr: negative name_len
 * - set of non-existing xattr: positive name_len, replaced false
 * - replacing an xattr: positive name_len, replaced true
 */
static int update_xattr_names_len(struct ngnfs_fs_info *nfi, struct ngnfs_transaction *txn,
				  struct ngnfs_inode_txn_ref *ino, int name_len, int replaced)
{
	u32 names_len;
	...
	names_len = le32_to_cpu(ino->ninode->xattr_names_len) + name_len;

	if (names_len > NGNFS_XATTR_MAX_NAMES_LEN) /* file system corruption */
		return -EUCLEAN;

	ngnfs_tblk_assign(ino->tblk, ino->ninode->xattr_names_len, cpu_to_le32(names_len));
...
int ngnfs_xattr_remove(struct ngnfs_fs_info *nfi, struct ngnfs_inode_ino_gen *ig, char *name)
{
	...
	do {
		...
		ret = ngnfs_inode_get(nfi, &txn, NBF_WRITE, ig, &ino) 				?:
		      ...
		      update_xattr_names_len(nfi, &txn, &ino, xa.name_len, xa.replaced);
